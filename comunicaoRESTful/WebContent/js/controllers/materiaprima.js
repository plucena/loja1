angular.module('App.controllers', []).controller('MateriaPrimaListController', function($scope, $state, /*popupService, $window,*/ MateriaPrima) {
  ...
  $scope.deletarMateriaPrima = function(materiaprima) { // Deletar matéria. Issues a DELETE to /api/movies/:id
  ...
  $scope.materiaprima = MateriaPrima.get({identificador: 'nome='+ $stateParams.nome }); //Get a matéria prima.Issues a GET to URI
  ...
  $scope.loadMateriaPrima = function() { //Issues a GET request to URI to get a MP to update
    $scope.materiaprima = MateriaPrima.get({identificador: 'nome=' + $stateParams.nome });
  };
